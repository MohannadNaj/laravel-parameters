module.exports = {
	methods: {
		...
		choseParamCategory(data) {
			...
			this.$http.post(window.Laravel.base_url + 'parameters/'+parameter_id+'/category/' + category_id)
			.then(response => {
				var updatedParameter = response.data.parameter;

				this.parameters = this.parameters.filter(x =>x.id != updatedParameter.id);
				...
				this.$nextTick(y => {
					...
					var msg = 'Parameter (' +
						...
						newCategory.title;
					EventBus.fire('changed-paramCategory', {data: data, parameter: updatedParameter, ok: true});
					this.alert(msg);
				});
			}).catch((error) => {
                var errorMessage = 'Error in updating parameter (' + parameter_id + ')';
