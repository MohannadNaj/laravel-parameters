module.exports = {
    methods: {
		...
		confirmedRemoveParameter(parameter) {
			this.$http.delete(window.Laravel.base_url + 'parameters/'+ parameter.id)
			.then((response) => {
				var data = response.data;

				var parameterIndex = this.parameters.indexOf(this.parameters.find(x => x.id == parameter.id));
			...
			.catch((error) => {
                var errorMessage = 'Error in removing parameter (' + parameter.id + ')';
