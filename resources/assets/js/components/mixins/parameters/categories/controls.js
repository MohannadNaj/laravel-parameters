module.exports = {
	methods: {
		extractCategoriesParameters() {
			...
			this.categoriesParameters = _.filter(this.parameters,
				parameter=> { return categoriesIds.indexOf(parameter.id.toString()) != -1 });

			_.each(this.categories, (category) => {
				category.parameters = _.filter(category.parameters, parameter => { return categoriesIds.indexOf(parameter.id.toString()) == -1 });
			});
		...
		enableEditCategoriesMode() {
			...
			var categoriesCategory = _.find(this.categories, category => { return category.isCategoriesGroup;});
			categoriesCategory.blocked = false;
			this.$nextTick(x=>{
				this.$refs[categoriesCategory.target+ "_parameter_category"][0].openCategory();
			});
		},
		disableEditCategoriesMode() {
			...
			this.$nextTick(x=> {
				this.openCategoryByHash(this.openedCategory == null || this.openedCategory == "" ? this.categories[0].target : this.openedCategory);
		...
		prepareCategory(key) {
			...
			if(categoryRelatedParameter == null || categoryRelatedParameter.value.length == 0)
				title = "Unnamed Category";
			else
				title = categoryRelatedParameter.value;
			...
			preparedCategory.parameters = _.filter(this.parameters, x => x.category_id == key);
			preparedCategory.relatedParameter = categoryRelatedParameter;
			return preparedCategory;
		},
    	getCategoriesRefs()
    	{
        	return _.keys(this.$refs).filter(x => x.indexOf("_parameter_category") > -1);
    	},
		...
		openCategoryByHash(hash = null){
	        ...
	        if(categories.indexOf(hash + "_parameter_category") > -1)
	        {
	        	...
	        	if(categoryToOpen != undefined)
	        		return categoryToOpen.openCategory();
	        }
