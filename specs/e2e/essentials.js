...
module.exports = {
  ...
  'Add Category': (browser) => {
    browser
      .assert.containsText('.btn.btn-default.btn-sm', 'Edit'); // edit categories button
    ...
    'Create Textfield Parameter': (browser) => {
        ...
        .setValue(`input[placeholder="parameter_name"]`,
            `testing-${(new Date()).getTime()}`) // parameter name
        .setValue(`input[placeholder="parameter_label"]`,
            `label of testing-${(new Date()).getTime()}`) // parameter label
        .setValue(`input[placeholder="parameter_label"]`,
            `label of testing-${(new Date()).getTime()}`) // parameter label
    ...
    'Change Created Parameter Category': (browser) => {
        browser
        .click('.panel .panel-footer .col-xs-4:nth(2) button') // change category button
    }
  };
